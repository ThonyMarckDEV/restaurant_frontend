...
import { BookOpenIcon, CurrencyDollarIcon } from '@heroicons/react/24/outline';

const MenuForm = ({ form, setForm, handleChange }) => {
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <h3 className="text-lg font-black text-slate-800 flex items-center gap-2 mb-4 border-b border-slate-100 pb-2 uppercase">
                <BookOpenIcon className="w-5 h-5" /> Configuración del Menú
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                {/* Nombre */}
                <div className="md:col-span-8">
                    <label className="block text-xs font-bold text-slate-700 uppercase mb-1">Nombre del Menú <span className="text-red-500">*</span></label>
                    <input
                        type="text"
                        value={form.nombre || ''}
                        ...
                        placeholder="Ej: Menú Ejecutivo, Menú Económico..."
                        required
                    />
                </div>
