...
const VentaForm = ({ 
    ...
    // Se calcula el total ignorando el estado 3 (Cancelado). Así no afecta la plata.
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col lg:grid lg:grid-cols-12 gap-6 mt-6">
            
            {/* 1. BUSCAR ORDEN */}
            <div className="order-1 lg:col-span-5 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-5 h-fit">
                <div>
            ...
            <div className="order-2 lg:order-2 lg:col-span-7 lg:row-span-2">
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 h-full overflow-hidden flex flex-col min-h-[500px]">
                            ...
                            <>
                                ...
                                <div className="flex-1 overflow-y-auto pr-2 space-y-1 mb-6">
                                    ...
                                    {ordenDetalle.detalles?.map((det, idx) => {
                                        ...
                                        return (
                                            <div key={idx} className={`py-2 border-b border-slate-50 flex justify-between items-start ${isCancelado ? 'opacity-50' : ''}`}>
                                                <div className="flex-1">
                                                    <div className="flex items-start gap-2">
                                                        {/* Cantidad */}
                                                        <span className={`font-mono text-xs font-bold px-1.5 py-0.5 rounded ${isCancelado ? 'bg-rose-50 text-slate-400 line-through decoration-rose-500 decoration-2' : 'bg-slate-100 text-slate-500'}`}>
                                                            {Math.round(det.cantidad)}
                                                        </span>
                                                        <div className="flex flex-col">
                                                            {/* Nombre tachado con línea roja */}
                                                            <p className={`text-sm font-bold leading-tight ${isCancelado ? 'text-slate-400 line-through decoration-rose-500 decoration-[1.5px]' : 'text-slate-800'}`}>
                                                    ...
                                                    {det.platos_menu?.length > 0 && (
                                                        <p className={`mt-1 pl-8 text-[11px] leading-tight ${isCancelado ? 'text-slate-300 line-through decoration-rose-300' : 'text-slate-400'}`}>
                                                            + {det.platos_menu.map(p => p.nombre || p.plato?.nombre).join(' + ')}
                                                        </p>
                                                    )}
                                                </div>
                                                <div className="text-right ml-4">
                                                    {/* Precio original visible, pero tachado con línea roja */}
                                                    <p className={`text-sm font-black font-mono ${isCancelado ? 'text-slate-400 line-through decoration-rose-500 decoration-[1.5px]' : 'text-slate-900'}`}>
                                ...
                                <div className="bg-slate-900 rounded-xl p-5 text-white flex justify-between items-end shadow-lg mt-auto">
                                    <div>
            ...
            {/* 3. MÉTODO DE PAGO Y BOTÓN */}
            <div className="order-3 lg:order-3 lg:col-span-5 space-y-6">
