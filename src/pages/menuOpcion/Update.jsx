...
import { useUpdate } from 'hooks/menuOpcion/useUpdate';
...
const Update = () => {
    ...
    return (
        <div className="container mx-auto p-6">
            <PageHeader title="Editar Opción" icon={PencilSquareIcon} buttonText="Cancelar" buttonLink="/menu-opcion/listar" />
            <AlertMessage type={alert?.type} message={alert?.message} details={alert?.details} onClose={() => setAlert(null)} />

            <form onSubmit={handleSubmit} className="max-w-3xl mx-auto space-y-6">
                <MenuOpcionForm form={formData} setForm={setFormData} handleChange={handleChange} />
                <div className="flex justify-end gap-4">
                    <button type="button" onClick={() => navigate('/menu-opcion/listar')} className="px-6 py-3 bg-slate-100 text-slate-600 rounded-lg font-bold hover:bg-slate-200 uppercase text-sm transition-all">Cancelar</button>
