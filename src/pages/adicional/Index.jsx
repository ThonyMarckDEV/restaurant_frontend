...
import { useIndex } from 'hooks/adicional/useIndex';
...
const Index = () => {
    const {
        loading, adicionales, paginationInfo, filters, alert, setAlert,
        ...
        { 
            ...
            render: (row) => (
                <div className="flex gap-2">
                    ...
                    <Link to={`/adicional/editar/${row.id}`} className="p-2 text-slate-500 hover:bg-slate-50 rounded-lg transition-all" title="Editar">
    ...
    return (
        <div className="container mx-auto p-6">
            <PageHeader 
                ...
                subtitle="Gestión de complementos para pedidos a la carta o menú"
                icon={PlusCircleIcon} 
                buttonText="+ Nuevo Adicional" 
                buttonLink="/adicional/agregar" 
            />
            ...
            <Table 
                ...
                pagination={{...paginationInfo, onPageChange: fetchAdicionales}} 
            />
            
            {confirmAction.show && (
                <ConfirmModal 
                    message={confirmAction.type === 'delete' 
                        ? "¿Estás seguro de eliminar este adicional? No podrá eliminarse si está registrado en alguna venta." 
                        : "¿Deseas cambiar el estado del adicional?"} 
                    onConfirm={handleConfirmAction} 
                    onCancel={() => setConfirmAction({show: false, id: null, type: ''})} 
                />
            )}
            
            <ViewModal isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Detalle del Adicional" isLoading={viewLoading}>
                {viewData && (
                    <div className="space-y-6">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                            <div className="flex items-center gap-4">
                                <div className="bg-slate-100 p-3 rounded-full">
                                    <PlusCircleIcon className="w-8 h-8 text-slate-500" />
                                </div>
                                <div>
                                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Adicional</h4>
                        ...
                        <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 max-w-xs">
                            <h4 className="text-[10px] font-bold text-slate-400 uppercase mb-1">Precio</h4>
